...
def _build_hint(
    ...
    if mode == "hint":
        ...
        action = (
            ...
            f"workflow deliverable. If yes, call "
            f"`meta_invoke(name=\"{skill_name}\")` as the first action; the "
            f"framework will drive the DAG and the deliverable becomes the "
            f"assistant reply. Do not answer directly in that case. Do not "
            f"call ordinary tools before `meta_invoke`; the meta-skill will "
            f"call its own sub-skills internally. If a direct answer is more "
            f"appropriate because the user is only asking a quick question or "
            f"asking about the workflow itself, ignore this candidate and "
            f"answer normally."
        )
    else:
        ...
        lead = evidence
        action = (
            f"For a concrete deliverable request that matches this workflow, "
            ...
            f"multi-step work product. Do not answer directly or call ordinary "
            f"tools such as web/search/http/file tools before `meta_invoke` in "
            f"that case; the meta-skill DAG will call any required sub-skills "
            f"internally and its deliverable becomes the assistant reply."
        )
    lines = [
    ...
    lines.append(
        ...
        "deliverable requests, prefer starting the matched workflow over "
